...
function formatCost(cost: ImagesModel<"openrouter-images">["cost"]): string {
	return `{
				input: ${cost.input},
				output: ${cost.output},
...
function generateImageModelsFile(models: ImagesModel<"openrouter-images">[]): string {
	const modelEntries = models
		...
		.map((model) => `\t\t${JSON.stringify(model.id)}: ${formatImageModel(model)},`)
		.join("\n");
	const providerEntries = `\topenrouter: {\n${modelEntries}\n\t},`;
